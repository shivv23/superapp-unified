...
function mapHoldingToPosition(h: Holding): Position {
  let type = "EQ";
  if (h.type === "mutual_fund") type = "MF";
  else if (h.type === "etf") type = "ETF";
  else if (h.type === "fno") type = "F&O";
...
function formatChgOI(val: number): string {
  const prefix = val >= 0 ? "+" : "";
  return prefix + val.toLocaleString("en-IN");
}

function formatIV(val: number): string {
  return val.toFixed(1) + "%";
}

function getCallBg(strike: number, spot: number, atmStrike: number): string {
  if (strike < spot && strike !== atmStrike) return "bg-blue-500/5";
  if (strike === atmStrike) return "bg-blue-500/10 border-x border-blue-500/20";
  return "";
}

function getPutBg(strike: number, spot: number, atmStrike: number): string {
  if (strike > spot && strike !== atmStrike) return "bg-blue-500/5";
  if (strike === atmStrike) return "bg-blue-500/10 border-x border-blue-500/20";
  return "";
}

function getStrikeBg(strike: number, atmStrike: number): string {
  if (strike === atmStrike) return "bg-blue-500/10 border-x-2 border-blue-500/30 text-blue-400 font-bold";
...
export default function FOTradingPage() {
  ...
  const fetchChain = useCallback(async () => {
    ...
    setChainError(null);
    try {
      const res = await apiGetOptionsChain("NIFTY");
      if (res.status === "success") {
        setChain(res.data);
        if (!selectedStrike && res.data.strikes.length > 0) {
          const atm = res.data.strikes.find((s) => s.is_atm);
          setSelectedStrike(atm ? atm.strike : res.data.strikes[Math.floor(res.data.strikes.length / 2)].strike);
        }
      }
    } catch (e: unknown) {
      setChainError(e instanceof Error ? e.message : "Failed to load options chain");
    } finally {
  ...
  useEffect(() => {
    async function loadHoldings() {
      ...
      try {
        const res = await apiGetHoldings();
        if (res.status === "success" && Array.isArray(res.data)) {
          setPositions(res.data.map(mapHoldingToPosition));
        }
      } catch {
        setPositions([]);
      } finally {
  ...
  const atmStrike = useMemo(() => {
    ...
    const atm = chain.strikes.find((s) => s.is_atm);
    return atm ? atm.strike : chain.strikes[Math.floor(chain.strikes.length / 2)]?.strike ?? null;
  }, [chain]);

  const selectedStrikeData = useMemo(() => {
    if (!chain || selectedStrike === null) return null;
    return chain.strikes.find((s) => s.strike === selectedStrike) ?? null;
  }, [chain, selectedStrike]);

  const greeksForDisplay = useMemo(() => {
    if (selectedStrikeData) {
      const g = selectedSide === "CE" ? selectedStrikeData.call_greeks : selectedStrikeData.put_greeks;
      return [
        { name: "Delta", value: g.delta.toFixed(4), color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20" },
        { name: "Gamma", value: g.gamma.toFixed(5), color: "text-purple-400", bg: "bg-purple-500/10", border: "border-purple-500/20" },
        { name: "Theta", value: g.theta.toFixed(2), color: "text-red-400", bg: "bg-red-500/10", border: "border-red-500/20" },
        { name: "Vega", value: g.vega.toFixed(2), color: "text-yellow-400", bg: "bg-yellow-500/10", border: "border-yellow-500/20" },
        { name: "Rho", value: g.rho.toFixed(2), color: "text-green-400", bg: "bg-green-500/10", border: "border-green-500/20" },
      ];
    }
    if (chain && atmStrike !== null) {
      const atm = chain.strikes.find((s) => s.strike === atmStrike);
      if (atm) {
        const g = selectedSide === "CE" ? atm.call_greeks : atm.put_greeks;
        return [
          { name: "Delta", value: g.delta.toFixed(4), color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20" },
          { name: "Gamma", value: g.gamma.toFixed(5), color: "text-purple-400", bg: "bg-purple-500/10", border: "border-purple-500/20" },
          { name: "Theta", value: g.theta.toFixed(2), color: "text-red-400", bg: "bg-red-500/10", border: "border-red-500/20" },
    ...
    return [
      { name: "Delta", value: "—", color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20" },
      { name: "Gamma", value: "—", color: "text-purple-400", bg: "bg-purple-500/10", border: "border-purple-500/20" },
      { name: "Theta", value: "—", color: "text-red-400", bg: "bg-red-500/10", border: "border-red-500/20" },
      { name: "Vega", value: "—", color: "text-yellow-400", bg: "bg-yellow-500/10", border: "border-yellow-500/20" },
      { name: "Rho", value: "—", color: "text-green-400", bg: "bg-green-500/10", border: "border-green-500/20" },
    ];
  }, [selectedStrikeData, chain, atmStrike, selectedSide]);
              ...
              <Card variant="glass" className="p-4">
                <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-lg font-bold text-white">{chain?.underlying ?? "NIFTY"}</span>
                  </div>
                  <div className="h-6 w-px bg-white/10 hidden sm:block" />
                  <div>
                    <span className="text-white/50 text-xs">Spot</span>
                    <p className="text-white font-semibold">{formatCurrency(chain?.spot_price ?? 0)}</p>
                  </div>
                  <div>
                    <span className="text-white/50 text-xs">Change</span>
                    <p className={`font-semibold flex items-center gap-1 ${(chain?.day_change ?? 0) >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                      {(chain?.day_change ?? 0) >= 0 ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />}
                      {formatCurrency(Math.abs(chain?.day_change ?? 0))} ({formatPercent((chain?.day_change_pct ?? 0) / 100)})
                    </p>
                  </div>
                  <div className="h-6 w-px bg-white/10 hidden sm:block" />
                  <div>
                    <span className="text-white/50 text-xs">Expiry</span>
                    <p className="text-white font-semibold">{chain?.expiry ?? "—"}</p>
                  </div>
                  <div className="ml-auto">
                    <Button variant="ghost" size="sm" onClick={fetchChain} disabled={chainLoading}>
                      {chainLoading ? (
              ...
              {chainLoading && (
                <Card variant="glass" className="p-12">
                  <div className="flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-white/50 text-sm">Loading options chain…</p>
                  </div>
                </Card>
              )}
              ...
              {chainError && !chainLoading && (
                <Card variant="glass" className="p-8">
                  <div className="flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-white/50 text-sm">{chainError}</p>
                    <Button variant="secondary" size="sm" onClick={fetchChain}>
                      <RefreshCw className="h-4 w-4 mr-1" />
                      Retry
                    </Button>
                  </div>
                ...
                <Card variant="glass" className="p-0 overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {chain.strikes.map((row: OptionsStrike) => {
                          const isSelected = row.strike === selectedStrike;
                          const callBg = getCallBg(row.strike, spot, atmStrike ?? 0);
                          const putBg = getPutBg(row.strike, spot, atmStrike ?? 0);
                          const strikeBg = getStrikeBg(row.strike, atmStrike ?? 0);
                          ...
                          return (
                            ...
                            >
                              <td className={`px-3 py-2.5 text-right font-mono text-white/70 ${callBg}`}>{formatOI(row.call_oi)}</td>
                              <td className={`px-3 py-2.5 text-right font-mono text-xs ${callBg} ${getChgColor(formatChgOI(row.call_chg_oi))}`}>{formatChgOI(row.call_chg_oi)}</td>
                              <td className={`px-3 py-2.5 text-right font-mono text-white/60 text-xs ${callBg}`}>{formatIV(row.call_iv)}</td>
                              ...
                              <td className={`px-3 py-2.5 text-left font-mono font-semibold text-white ${putBg}`}>
                                ...
                                >
                                  {formatCurrency(row.put_ltp)}
                                </button>
                              </td>
                              <td className={`px-3 py-2.5 text-left font-mono text-white/60 text-xs ${putBg}`}>{formatIV(row.put_iv)}</td>
                              <td className={`px-3 py-2.5 text-left font-mono text-xs ${putBg} ${getChgColor(formatChgOI(row.put_chg_oi))}`}>{formatChgOI(row.put_chg_oi)}</td>
                              <td className={`px-3 py-2.5 text-left font-mono text-white/70 ${putBg}`}>{formatOI(row.put_oi)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                ...
                <div className="overflow-x-auto">
                  {holdingsLoading ? (
                    <div className="flex flex-col items-center justify-center gap-3 p-12">
                      <Loader2 className="h-8 w-8 text-blue-400 animate-spin" />
                      <p className="text-white/50 text-sm">Loading holdings…</p>
                    </div>
                  ) : positions.length === 0 ? (
                    <div className="flex flex-col items-center justify-center gap-3 p-12">
                      <AlertTriangle className="h-8 w-8 text-amber-400" />
                      <p className="text-white/50 text-sm">No holdings found.</p>
                    </div>
                  ) : (
                    <table className="w-full text-sm">
                      <thead>
                          ...
                          <tr key={idx} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                            <td className="px-4 py-3">
                              ...
                              {pos.name && pos.name !== pos.symbol && (
                                <span className="block text-white/40 text-xs">{pos.name}</span>
                              )}
                            </td>
                            <td className="px-4 py-3">
                              <Badge variant={pos.type === "CE" ? "info" : pos.type === "PE" ? "warning" : pos.type === "F&O" ? "gold" : "default"} className="text-xs">
                                {pos.type}
                              </Badge>
                            </td>
